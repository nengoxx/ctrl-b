...
- **The app IS this repo (ctrl-b v1.0)** — a mobile-first React/TS/Vite **PWA** backed by a
  ...
  **Doc map — read these before designing or implementing a feature:**
  ...
  | [`docs/UPDATE_PLAN.md`](./docs/UPDATE_PLAN.md) | The update/migration architecture (config-shape migration + `update.sh`; **✅ built, proven over every release since**). Read before touching `app/config_migration`, `install.sh` or the release path. |
  ...
  | [`docs/MEDIA_MANAGER_PLAN.md`](./docs/MEDIA_MANAGER_PLAN.md) | Phase 21 — the media manager (D65, **✅ shipped v1.7.7**): the write API, the per-destination art LIBRARY model, crop + focal point, drag reorder. **The authority on writes/libraries/management UI** (MEDIA_PLAN keeps namespaces/roles/serving). §12 = the S0–S6 slice ladder. Read before touching the media write path, the gallery, or anything that resolves "which image is live". |
  | [`docs/ATTACHMENTS_PLAN.md`](./docs/ATTACHMENTS_PLAN.md) | Phase 22 — composer attachments (D68, **✅ built, UNRELEASED — rides v1.7.8**): staged uploads, `AttachmentPart`, `read_attachment`, the no-vision strip, bubble image display. §11 = the council record; §9 = the S0–S6 ladder. Read before touching the composer send path, the attachment store, or `_assemble`'s content shapes. |
  | [`docs/ROLEPLAY_PLAN.md`](./docs/ROLEPLAY_PLAN.md) | Phase 23 — characters are agents + lorebooks (D70, **S0–S6b CLOSED; S7 rides the owner's regular use**): the AgentDef expansion, the universal Voice/Duties assembly, V2/V3 card import, the lorebook subsystem, the agent gallery + three-state backdrop. §13 = the council record; §10 = the S0–S7 ladder. Read before touching `AgentDef`, prompt assembly, the agents surface, or anything lorebook-shaped. |
  | [`docs/LIVE_VOICE_PLAN.md`](./docs/LIVE_VOICE_PLAN.md) | Phase 24 — live voice mode / call mode (D71, **✅ S0–S4 BUILT, the phase gate CLOSED 2026-09-26 — `voice.live.enabled` ON since v1.7.8; the D72/D73/D74/D76/D77 waves rode it**): the Speaches-realtime ear behind the relay, the first WebSocket (D71's narrow media-ingress admission), client-submitted turns, C3 as the mouth, energy-gated barge-in. §9 = the council record; §7 = the S0–S4 ladder; §2.1 = the reference-project inheritance ledger. Read before touching the voice subsystem, `audioController`, or anything WebSocket-shaped. |
  ...
  | [`docs/DEPLOY_EMMA.md`](./docs/DEPLOY_EMMA.md) | emma's **topology (D32) + the as-executed record** of the 2026-07-10 first deploy. **The living procedures are [`deploy/linux/README.md`](./deploy/linux/README.md)** (install · §Release · §Rollback) — go there to *do* anything; come here for why the layout is what it is. |
...
- **The dev + deploy host is emma (Ubuntu Linux) as of the 2026-07-10 v1.0.0 deploy** — sessions run
  ...
  (`fleet._ping_cmd` ping syntax · `run_shell` shell · `core/fsutil.fsync_dir` no-op · `check.py`),
