...
Steps, each flag-gated so you can hand any of them to the owner:
  ...
  1. secret          — SFTP the gitignored dashboard_v2/config.yaml -> ~/.ctrl-b/config.yaml (0600)
  ...
  3. install (prod)  — deploy/emma/scripts/install.sh prod (prereq check + 3.14 venv + npm build + render units)
...
Usage (Bash tool needs dangerouslyDisableSandbox for LAN):
    py -3 deploy/emma/bootstrap.py --dry-run        # show the plan, do nothing (no connection)
    ...
    py -3 deploy/emma/bootstrap.py --no-prereqs     # skip sudo (you ran apt/linger/operator yourself; non-apt distro)
...
def ensure_prod_tree(m: "Emma", prod_repo: str, dev_repo: str, gh: str | None) -> int:
    ...
    if state == "full":
        ...
        print(f"      bash {dev_repo}/dashboard_v2/deploy/emma/scripts/migrate-layout.sh   # if a dev tree exists")
        print(f"      bash {prod_repo}/dashboard_v2/deploy/emma/scripts/migrate-layout.sh  # otherwise")
...
def main() -> int:
    ...
    try:
        ...
        if not START_AGENT:
            print(f"  Start the agent when ready:  ssh {host} -t 'cd {dev_repo}/dashboard_v2 && bash deploy/emma/scripts/start-claude.sh'")
        return 0
    finally:
        m.close()
