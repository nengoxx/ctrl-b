...
- Uses the `openai` SDK against the configured backend (`local` = llama.cpp `llama-server`
  `/v1`, or `cloud` = OpenRouter et al.). Streams tokens out over SSE.
- Tools = the action registry. The model proposes an action; high-risk/`confirm` actions are
  **not auto-executed** — they surface as a **command/action bubble** (Vapor `.b.cmd`) with
  execute / edit / dismiss, mirroring today's review-before-send flow. Low-risk actions can run
...
- **Memory** (see §4) is injected into the system prompt per request; managed from Conf.
...
Message   id, thread_id, role(user|assistant|system|tool), content, ts, meta      [SQLite]
...
## 4. Agent memory (open detail — recommendation)

Hybrid, managed from the Conf tab:

- **Rolling summary** — periodically summarize older turns into a `Memory(kind=summary)` row so
  long threads stay within context without losing gist.
- **Pinned facts** — `Memory(kind=fact, pinned=true)` for durable prefs ("my GPU box is titan",
  "default to local inference"). Always injected.
- Conf tab lists memory items with delete/pin and a "clear thread / clear all" control. This is
  the "memory management in settings" requirement.
...
- **Tabs** = Vapor's four: **Fleet** (hero + device rows w/ expandable services + fleet
  ...
  (inference/STT/TTS endpoints, hosts CRUD, prompt files, server, memory mgmt, appearance).
- **Composer** (shared by Fleet + Agent): textarea + push-to-talk mic + send. `$`/`>` prefix →
  guarded exec; `k:`/`o:` → force local/cloud (port the old prefixes); else → agent.
- **Command/action bubbles** (`.b.cmd`): execute / edit / dismiss; high-risk actions confirm.
