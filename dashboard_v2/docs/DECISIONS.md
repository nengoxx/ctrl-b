...
**Clean production = sparse-checkout, NOT a divergent branch or a separate repo.** The prod clone runs `git sparse-checkout set dashboard_v2` (cone mode — the modern `set` default — so the app tree + all top-level files like AGENTS.md/CLAUDE.md are present; only the legacy prototype DIRS `ws_claude*`, `ctrl-b (Vapor)`, `wol_server`, `ws_codex*` drop out). Those folders stay in history but never appear in the prod tree — **zero merge conflicts** (nothing is deleted, just not checked out). Rejected: a `prod` branch that deletes prototypes (every dev→main merge re-collides — known anti-pattern); a separate repo (fragments history, double remotes). The eventual permanent cleanup is a **one-time** delete commit on `main` when v2 fully replaces the live Flask app ("graduation"), not an ongoing divergence.
...
**Status.** LOCKED 2026-06-29. Supersedes the earlier single-tree, shared-backend deploy sketch (dev was Vite-only proxying to the prod backend → NOT isolated). Artifacts, tidy under `deploy/emma/`: `bootstrap.py` + `README.md` at top, `systemd/` (the three units), `scripts/` (`install.sh [prod|dev]`, `serve-https.sh`, `start-claude.sh [session] [dir]`, `migrate-layout.sh`, `add-dev-worktree.sh`); rationale `docs/DEPLOY_EMMA.md`.
