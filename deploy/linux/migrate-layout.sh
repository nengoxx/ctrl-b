...
#   ~/github/ctrl-b      → CLEAN sparse, tag-pinned PROD clone (cone mode: backend/frontend/deploy + top-level
#                          files; docs/, design/, archive/, tools/, agents/, skills/ are excluded from prod)
...
# Cone mode (the modern `set` default): args are DIRECTORIES; top-level files (AGENTS.md, CLAUDE.md, config.yaml,
# …) are always included. So this = the app trees + root files; docs/design/archive/tools/agents/skills drop out.
SPARSE=(backend frontend deploy)
...
echo ""
echo "✓ Migration done."
echo "   PROD: $PROD   (sparse$( [ -n "$TAG" ] && echo ", $TAG" || echo ", main" ))   → bash deploy/linux/install.sh prod"
echo "   DEV : $DEV   (branch dev, full)               → bash deploy/linux/install.sh dev   + start-claude.sh"
echo "   Re-launch the agent IN THE DEV TREE:  bash $DEV/tools/start-claude.sh"
