...
- **Adoption = warn-first baseline → burn down to clean before deploy**; the Prettier reflow lands as its own commit (never mixed with logic). Ratchets (post-baseline): pyright `strict`, `@eslint-react` broad ruleset, stylelint (already owned by the theme-engine hardening slice, `THEME_ENGINE` §14.13).
