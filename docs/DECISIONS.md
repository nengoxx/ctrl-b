...
`banner` pool's first member (`banner.webp` joined that pool, LEADING it, so the shipped look is
unchanged) and reads this ladder only as its empty-pool fallback, so a kit background moves the
backdrop and not the carousel. The PIN survives on
...
## D79 — Export is the import read backwards: SillyTavern-compatible card PNG + JSON, standalone lorebook JSON, the referenced-by line ✏️ RULED 2026-09-26 (main seat, from the owner in conversation — *"we totally forgot to create an export button to export the card in PNG format as silly tavern and other project do … same for the lore books"*, *"the character could be exported with the lore book, or without (just unlink the lore book before exporting)"*, *"don't delete lore books automatically at all, maybe say inside the lore book which characters reference it"*; evidence = R65 §1.6/§5.2 · R66 §1/§2.5 · R87 RP-4/RP-8; design of record = [`ROLEPLAY_PLAN.md`](./ROLEPLAY_PLAN.md) §15; council = blind Maya design round BUILD WITH CHANGES [4 HIGH · 3 MED · 2 LOW — all folded] → confirm ALL RESOLVED + 3 guards folded → blind OPUS 5.5 nuance round BUILD WITH CHANGES [3 HIGH · 8 MED · 7 LOW — 17 folded, F2 closed ISS-26's memory-nudge arm on evidence: `memory` is a core tool every character holds]; **✅ BUILT 2026-09-26 (the clean session; two pinned-Opus lanes) + the two-reviewer code round CLOSED — blind Opus 5.5 [1 MED · 7 LOW] ∥ blind Maya [1 MED]; Opus F1 corrected §15.4's ST-dialect rule for card-origin books (the stashed `extensions` is LIFTED to ST names, not dropped); both confirms CONFIRMED SHIP; as-built = ROLEPLAY_PLAN §15.12**)

- **The export composes from the CURRENT character** (what SillyTavern does) and the `card.json` sidecar supplies every field we never mapped; `strip_executable` runs on the way out on both the envelope and the card, apart, as the import strips them. The SOUL's original three-way split is restored only when the SOUL still equals the import's composition under the SOUL writer's CRLF→LF fold (measured: both dev cards need the fold).
- **The server owns the card, the client owns the pixels:** the backend has no image codec by policy (D65), so the app re-encodes the bound avatar to PNG through its one canvas chokepoint and POSTs it as the carrier; the server validates the carrier structurally (signature · IHDR first and 13 bytes · lengths in bounds · CRCs · IEND zero-length and terminal), strips old card chunks, and writes both `chara` (V2 projection) and `ccv3` (V3) `tEXt` chunks — ST's own writer behaviour. `GET …/card` is the JSON seam.
- **A card embeds the books LINKED on the agent** (one verbatim; several merged into one named after the character); global books never. Unlink to export without — the character and the book stay different things.
- **Lorebook export = ST's standalone world-info object** (the dominant convention at the boundary), from one per-entry serializer with two dialects (standalone / spec `character_book`); a stashed ST position is re-emitted only while it still agrees with the entry's current position. The importer records a downgraded ST position under `extensions.position` (ST's own home) so the round trip is lossless.
- **Books are never deleted by a cascade.** The list carries `used_by` (agents + global) and the delete confirm names them. Deleting a character removes its default memory directory and reports what it kept.
- Polish rulings recorded with it: ISS-23 rides the duties flip (transition-only); ISS-27 matches ST's ASCII-boundary whole-word rule, includes speaker names behind `lorebooks.include_names` (default on), and lets `constant` bypass the secondary gate (R65 §1.5's precedence, the main seat over the reviewer's reading); ISS-29 lists agents under the minted grammar.
