...
**① The release, as executed.** Pre-flight this session: the local gate `check.py --e2e` 7/7 at `4ce70c3` on a fresh `TMPDIR` (pytest 362 s · FE 84 s · e2e 177 s); the prod config migration dry-run + apply on a FRESH copy = only step 3 (`wake.presence_device_ips` → `presence_devices`), stamped 5, and the loaded result had the ear ON + PocketTTS + the one presence device; the `~/.cache/tmp` cleanup the 43rd session left running had NOT finished (339k entries) — re-run, now 34k. **Backups (rollback = restore ONE of these FIRST, then §Rollback to v1.7.7):** the pre-release copy `~/.ctrl-b/config.yaml.20260926T151736Z.pre-v1.7.8` (0600, stamped 2) and the migrator's own `~/.ctrl-b/backups/config.yaml.20260926T154424Z`; DB snapshot `~/.ctrl-b/backups/ctrlb-20260926-174423.db.gz`. The tag's release notes name the payload: Phase 24 live voice (ear ON by default) + the D74–D78 call/agent waves, Phase 23 S8 (the persona library), Phase 22 composer attachments (D68), the R86–R89 audit fixes; 320 commits over v1.7.7. **Doc-truth sweep with the release:** the "UNRELEASED — rides v1.7.8" stamps flipped to "shipped v1.7.8" in CLAUDE.md's doc map, ATTACHMENTS_PLAN's header, ROLEPLAY_PLAN's header + §14's build line + the migration note, TODO's Phase 22 header/closer + the S8 line, UPDATE_PLAN §12a rows 3–5 + the rollback sentence. Dated historical notes that say "unreleased" in their own time (MEDIA_MANAGER_PLAN's step-2 amendment, CORE_MEMORY_PLAN's run log, DECISIONS's fold amendment) were left as provenance.

**▶▶ THE OWNER'S PROD ROUNDS (prod = https://emma.lobster-vector.ts.net, v1.7.8; RELOAD the PWA first — the SW is prompt-mode, accept the update prompt):**
1. **A first prod CALL** — Media route, loudspeaker, a few turns. The ear is on in prod for the first time; `voice.live.debug` is OFF there, so no readout and no trail. If anything misbehaves, flip Conf › Voice · Live › "Call debug readout" ON and call again: the trail lands in `~/.ctrl-b/calls/` (prod's `$CTRLB_HOME`), read with `ls -t ~/.ctrl-b/calls | head -1`.
2. **The mouth-wait card** (the 42nd block's four arms: talk over `thinking` · TV during `thinking` · headphones with interruption OFF/ON · mute mid-sentence) — on prod.
3. **The D75-amendment card** (the 39th block: the "default" pill on the gallery cards · `/new` with a default set vs none · the tools menu's root row) — on prod.
4. **Conf › Roleplay › Personas** — prod has NO personas and no agents dir: add one, link it on an agent ("Your persona" on the agent form), check `{{user}}` in a reply.
5. **The owed arms, on the owner's regular use:** the car on the CLEAN route (then `min_final_ms` 200 → 300 if home-side noise words persist) · the TV/other-room arm · ISS-19 (the Honor battery setting FIRST, then the lock arm, then the discard toast).

**Ops notes:** the local gate and the two hook gates (main push, tag push) ran serially on purpose — a push during the local gate is the 6-parallel contention flake class. `gh run watch <id> --exit-status --interval 30` in a background task is the clean way to wait on CI. `update.sh` was on the prod tree's disk (it ships since v1.3.x), so the plain form worked; its output names the config backup and the rollback caveat. Dev units are still RUNNING (`voice.live.debug` ON on dev, schema 5) — stop them when the owner is done poking. This session's scratch: `~/.cache/tmp/gate-s44/` (the gate's TMPDIR, disposable).

## Prior state (2026-09-26, FORTY-THIRD session — **THE TWO OWED WAVES, BOTH BUILT AND COUNCIL-CLOSED THE SAME SITTING. ① The mouth-wait CODE round: Emma's four findings built by the main seat (`ba654a0`: the owner's gesture WINS over a held start — `releaseHeldPastGate`/`pastGate` in `transport()`, a seek drops the slot; `final` clears the noise verdict on every exit; the disk pin; the two door comments), six new tests all red-proven, gate 6/6, dist rebuilt → Emma's self-contained confirm: SHIP, all four CONFIRMED. ② PHASE 23 S8 — THE PERSONA LIBRARY + THE NEUTRAL FRAMING (D78 + the D70 amendment) BUILT by two pinned-Opus lanes on disjoint files against a written API contract (backend+docs · frontend), main-seat audited (one fix: new-wins before the mint), blind Emma CODE round DO NOT SHIP [2 HIGH · 3 MED · 1 LOW] → ruled (F1/F2/F4/F6 accepted, F3 rejected as the engine's standing rule, F5 overruled: the schema admits what the one mint emits) → the fix wave built + red-proven → Emma's confirm: SHIP, no new findings → COMMITTED `776d616`. DEV MIGRATED to schema 5 for real (Ari = the default persona; the two blanks stayed blank links; backup `~/.ctrl-b-dev/config.yaml.20260926-pre-step5` + the tool's own). The prod-copy dry-run: only step 3 applies, stamped 5. ISS-29 + ISS-30 recorded. ▶▶ NEXT = ③ THE RELEASE — everything below is ready; it starts with a PUSH, which needs the owner's word.**)

**① The mouth waits — the code round, closed (record: LIVE_VOICE_PLAN §7's "THE MOUTH WAITS" block, F1/F2 sentences; the 42nd block below has the four findings as ruled).** F1: `transport()`'s resume releases a start held on the mouth gate and runs it PAST the gate (`releaseHeldPastGate` → `pastGate`, a one-shot synchronous bypass `mouthMayOpen` honours; a parked replay's `playNext` runs the same way); `seekChunked` nulls the slot (the seek owns the queue). F2: one re-entry of the `final` arm with `noiseOpen` cleared, so every exit runs without the verdict. F3: `test_settings_7a` pins `noise_verdict_ms` on disk. F4: the two call-door comments describe the boundary, not the deleted kill. Tests: `audioController.test.ts` "THE MOUTH WAITS" ×11 (4 new: hold→pause→resume · hold→pause→seek→gate-open · gate-open while paused → resume · seek to a pending chunk → landing → gate-open — the last two from Emma's coverage note) · `useLiveCall.test.ts` +1 (`speechStart → segmentNoise → final`, three final shapes + the held-ear drop). Reviews at `~/.cache/tmp/ctrlb-session43/emma-lc1-confirm.md` (+ the 42nd session's review beside the prompt). **The owner's phone card for it is the 42nd block's, unchanged (dev :8443, reload first).**

**② S8 — as built (record: ROLEPLAY_PLAN §10's S8 block + §14.5 the code round; D78 + the D70 amendment headers → built; ISSUES ISS-21/ISS-25 → fixed; TODO Phase 23 S8 ticked; UPDATE_PLAN §12a row 5; SECURITY_MODEL §2.9; DESIGN's `AgentDef` list).** *BE:* `PersonaCfg` + `RoleplayCfg.personas`/`default_persona` (keys pinned by `is_persona_slug`, `^[a-z0-9][a-z0-9_-]*$` — the mint's grammar, deliberately) · `AgentDef.persona` replaces `user_name` (the OWNER's persona — the comment says so, because the file calls SOUL.md "persona" everywhere else) · ONE resolver `services/agent/persona.py::resolve_persona` (link → default → none; a dangling slug falls through, warns once per (agent, slug)) feeding `macros_for` + `_persona_block` · `api/personas.py` POST (server mint via `mint_slug`, 409 taken, 422 empty) / PUT `/{slug}` (slug fixed) / DELETE `/{slug}` (no cascade) on `api/hosts.py`'s write path · **the settings PUT refuses `roleplay.personas` with 422** (Emma F1 — a deep-merge would resurrect a deleted persona; `computers` is the same unguarded class = ISS-30) · migration **step 5** (`VERSION` 5): global → `personas[mint]` + `default_persona` (key presence, not truthiness — Emma F2), `user_name` renamed in place in `agent.defaults` + every agent file (exact-name reuse else the mint's suffix walk; explicit blank stays blank; new-wins decided BEFORE the mint), `roleplay.persona` consumed in every empty shape incl. `""` (Emma F4), a non-blank scalar refused; `roleplay.persona` retired from the env grammar · Part B: `lorebook_intro` = "Lorebook — the entries that match what is being talked about:", `persona_intro` = "The owner's persona — who you are talking to:", both stamps change, a test pins that neither contains "not instructions". *FE:* `useRoleplay.ts` (types · `personaChoices` = the ONE options source for both selectors incl. `missing: <slug>` · three write hooks invalidating `["settings"]`) · `RoleplayEditor.tsx` — the character-tools draft stays; `PersonasCard` (add row with the inline 409 · "Default persona" `Seg` saved immediately · one `PersonaRow` per persona on `AgentRow`'s seeded-snapshot rule with its own PUT, remove via `requestConfirm`) · `AgentsEditor.tsx` "Your persona" select (shown when the library is non-empty OR the link is set, regardless of `rpShow`; `missing:` for a dangling link) · nothing under `frontend/` reads `user_name`/`roleplay.persona`. *Tests:* `test_personas_s8.py` (12) · `test_config_migration_step5_personas.py` (28 cases incl. the blank-default + both-shapes arms) · re-aimed `test_roleplay_s0/s1/s3`, `test_prompts_registry_p18` · FE `roleplayPersonas.test.tsx` (16) + `agentRoleplayFields.test.tsx` (+5) + `useAgents.test.ts`. *Dev:* `voice.live.debug` still ON; **the two imported agents keep a stale `card:` key** (inert); dist REBUILT by the FE lane (newer than every source). *Lane reports + all prompts/reviews:* `~/.cache/tmp/ctrlb-session43/` (disk).

**▶▶ ③ THE RELEASE (v1.7.8) = the 41st block's steps with these deltas — START HERE:**
1. **PUSH on the owner's word** (this session's commits are UNPUSHED: `ba654a0` · `776d616` · this handoff) → CI green on the tip (`gh run list --branch main --limit 1`).
2. **CONFIG BACKUP FIRST** (`cp ~/.ctrl-b/config.yaml ~/.ctrl-b/config.yaml.$(date -u +%Y%m%dT%H%M%SZ)`) — v1.7.8 now carries migrations **2→5**; on the prod copy only step 3 applies (`wake.presence_device_ips` → `presence_devices`) and steps 4/5 are no-ops (no `voice.live`, no roleplay block, no agents dir), the file stamped 5 — dry-run clean THIS session (`~/.cache/tmp/ctrlb-session43/mig/prod/`). Rollback = restore that backup, then v1.7.7.
3. Pre-deploy gate `backend/.venv/bin/python tools/check.py --e2e` locally — **RAN GREEN THIS SESSION at `776d616`: 7/7 incl. e2e (playwright), on a fresh `TMPDIR`; the handoff commit after it is docs-only.** **⚠ `~/.cache/tmp` had 548k stale entries this session (pytest could not create its numbered dirs → one env-only red); a cleanup of entries older than a day was started — check `ls -f ~/.cache/tmp | wc -l` is small before the gate, or pass a fresh `TMPDIR`.**
4. Tag `v1.7.8` on the verified sha → push the tag → wait for the CI release gate → re-pin `~/apps/ctrl-b` → `install.sh prod` (runs `--apply`) → `git describe --exact-match` + `/api/health` version `1.7.8`.
5. After: a first prod CALL (the ear turns ON in prod), the mouth-wait phone card (42nd block) on prod, then Conf › Roleplay › Personas (prod has none — add one, link it on an agent, check `{{user}}`), then the owed arms (car clean route · TV · ISS-19).

**Ops notes:** the S8 build ran as TWO Opus lanes in parallel on disjoint files with the API contract written into both briefs — both reported "contract unchanged/fits" and the halves met without a seam fix; keep the pattern. The full gate's pytest leg went red ONCE for a purely environmental reason (`could not create numbered dir … after 10 tries` — `/home/emma/.cache/tmp` had ~548k entries, mostly anonymous `tmp*` leftovers from gates); the suite re-ran green on `--basetemp=<fresh dir>`; the cleanup (`find ~/.cache/tmp -maxdepth 1 -mtime +1 -exec rm -rf`) was still running at close — verify. A stale session-42 monitor loop (`while pgrep hermes…; do sleep 30`) was found alive 4 h later and killed — a `-z` monitor must have a timeout. Emma lane: three self-contained runs this session (~8–15 min each), all `-p emma -m gpt-5.6-sol-900k --reasoning high --ignore-rules -t file,terminal -z`. Bash heredoc + python `s.index('\n\n', i)` bit twice on a file with no trailing blank line — append, don't anchor on the next paragraph. Dev units RUNNING (restarted on schema 5).

## Prior state (2026-09-26, FORTY-SECOND session — **THE THREE OWNER QUESTIONS RULED, ONE BUILT, TWO DESIGNED. Cut short by the usage limit (owner's word: "hand off for a clean session as we were"). (a) The barge-in/iron-rule question → "THE MOUTH WAITS" (a second D71 amendment): the reply is never killed for sound at reply-ready — it WAITS at the silent→audible doors until the ear settles, a noise verdict (`noise_verdict_ms`, 1000) lifts the hold early, `barge_in` stays out of it → blind Emma design round SHIP WITH CHANGES [2 MED, folded] → BUILT by one pinned-Opus lane (gate 6/6, dist rebuilt) → main-seat audit clean → COMMITTED as-built `68bc0b4` → **Emma's blind CODE round: SHIP WITH FIXES [2 MED · 2 LOW — ruled in ①, the first wave next session]**. (b) ISS-21 + (c) ISS-25 → R90 bought (framing + persona management, nine peers) → D78 (personas are a LIBRARY, the link on the agent, one resolver, rename-stable slug, a `personas` router, migration step 5) + the D70 framing amendment (neutral noun labels, no per-book switch) → blind Emma design round SHIP WITH CHANGES [6 MED · 1 LOW, all folded] → **UNBUILT: ROLEPLAY_PLAN §14 IS the build brief.** ▶▶ NEXT SESSION = ① close the mouth-wait code round → ② build S8 from plan §14 → ③ THE RELEASE (now migrations 2→5).**)

**① The mouth waits — as built (record: DECISIONS D71's second 2026-09-26 amendment; LIVE_VOICE_PLAN §4.1/§4.2/§4.3/§5.1 + the §7 "THE MOUTH WAITS" block; TODO Phase 24 ticked).** `audioController.ts`: the call's THIRD hook `setCallMouthGate` + ONE held continuation + `pokeCallMouth`; `playNext` asks at the opening edge and the resume out of a synthesis gap (`resuming = s.waiting && status === "loading"` — the seek latch and the owner's resume tap are NOT gated, design A1), `playWhole` asks after the retag wait; `reset`/`beginMessage` drop a held start without running it; `setCallMouthGate(null)` runs nothing. `useLiveCall.ts`: the `playbackStarted` kill is GONE (no `energyMs`/`minFinalMs` on the signal); pure `mouthMayOpen(s) = !waitingFinal && (!userSpeechActive || noiseOpen)`; `segmentNoise` → `noiseOpen` (normalized off when the segment closes, beside `probeOpen`); `speechStop` raises `waitingFinal` ONLY after an accepted start (Emma A2 — the mute→unmute strand); the verdict timer is armed on an accepted `speech_started`, fenced on leg + meter-epoch seq + `gen`, judges with the existing `tooQuiet`, cleared when no segment is open / on release / teardown; the poke runs at the end of every `send`. Knob: `LiveCfg.noise_verdict_ms` (1000, 0–5000, 0 = never classify) → `/voice/status` → Conf › Voice · Live "Noise verdict (ms)" (placed before "Min speech energy hold" — `confVoiceStt.test.tsx` pins the gate six directly after that row) → `config.example.yaml` (the LiveCfg-inventory test). Tests: `audioController.test.ts` "THE MOUTH WAITS" ×7 · `useLiveCall.test.ts` "the mouth WAITS" ×7 · `useLiveCallWiring.test.ts` "THE MOUTH'S GATE, wired" ×3 + three noise-verdict arms · BE status/default/bounds + round-trip. **Emma's CODE round (SHIP WITH FIXES; review at `~/.cache/tmp/ctrlb-session42/emma-lc1-code-review.md`, brief beside it) — RULED, all four ACCEPTED → ✅ BUILT by the main seat in the 43rd session as `ba654a0` (F1 = `releaseHeldPastGate`/`pastGate` in `transport()` + `seekChunked` drops the slot · F2 = one re-entry of the `final` arm with `noiseOpen` cleared · F3 · F4; six new tests incl. the two from Emma's coverage note, all red-proven; gate 6/6; dist rebuilt; Emma's self-contained CONFIRM round = SHIP, all four CONFIRMED, no new findings — `~/.cache/tmp/ctrlb-session43/emma-lc1-confirm.md`). The findings as ruled, for the record:** **F1 MED** an owner resume/seek does not supersede a held start — with a chunk landed behind a closed gate, pause + tap resume stays gated (`transport()` returns on `s.waiting`), and a seek to a ready chunk leaves the stale continuation alive to `playNext()` over the owner's pick when the ear settles → the owner's gesture WINS: `transport()`'s resume consumes the held continuation as a one-shot bypass, `seekChunked` cancels the slot; tests hold→pause→resume and hold→pause→seek→gate-open. **F2 MED** `final` does not clear `noiseOpen` (the lane's deviation 4) — a final landing before/without `speech_stopped` leaves the verdict alive → clear it on EVERY `final` exit (a later segment is a new epoch with its own timer, so nothing re-closes); test `speechStart → segmentNoise → final`. **F3 LOW** `test_settings_7a.py` must pin `noise_verdict_ms` on DISK beside `min_final_ms: 350`. **F4 LOW** two comments still describe the deleted kill (`store/liveCall.ts:35-36`, `tests/theme-engine/micGesture.test.tsx:554`) — keep the dismiss-before-prime rule, rewrite its rationale. Everything else Emma checked matched the design (doors, the one slot, `loading` honesty, A2/A3, the timer fences, the knob end to end, the docs). Then the owner's phone card below.

**② The persona library + the neutral framing — designed, council-closed, UNBUILT (record: DECISIONS D78 + the D70 amendment "the lorebook + persona framings carry NO authority claim"; ROLEPLAY_PLAN §14 = the whole design incl. §14.4's six folded amendments; ISSUES ISS-21/25 → RULED, build pending; TODO Phase 23 S8; evidence R90).** The shape in one breath: `roleplay.personas: {slug: {name, description}}` · `roleplay.default_persona` · `AgentDef.persona` (the link ON THE AGENT, any agent) · `resolve_persona` feeds both `{{user}}` and the persona block · the slug is minted ONCE server-side by `mint_slug` (card_import's, its third caller) and never changes on rename · writes via a `personas` router mirroring `api/hosts.py` (POST/PUT/DELETE, no cascade; reads on the settings doc; `default_persona` saved immediately through the settings PUT) · Conf per-row cards (Automations posture, `AgentRow`'s seeded snapshot) · the agent form's `user_name` input → a persona select that also shows a dangling `missing: <slug>` · **migration step 5 (`VERSION` → 5)**: `roleplay.persona` → `personas[mint]` + `default_persona`; `user_name` renamed IN PLACE to `persona` in `agent.defaults` AND every agent file (exact-name reuse, else mint; an explicit blank stays blank). Part B: `lorebook_intro` → `"Lorebook — the entries that match what is being talked about:"`, `persona_intro` → `"The owner's persona — who you are talking to:"`, descriptions rewritten (the boundary is the gate in code), `test_prompts_registry_p18.py` re-aimed, SECURITY_MODEL §2.9 one sentence, both stamps change; Core Memory's framings untouched (an owner call for Phase 19). **Build = ONE pinned-Opus lane briefed from plan §14 (Part A then B)** → main-seat audit → blind Emma CODE round → fix wave → full gate → `npm run build` → **dev migration** (`CTRLB_HOME=~/.ctrl-b-dev backend/.venv/bin/python -m app.config_migration --apply` from `backend/` — dev's global persona becomes its default persona; lynette's `user_name: ''` → `persona: ''`) → commit → the §10 S8 as-built block + the §3.2/§4.2/§6.4 body text move with the code.

**③ THE RELEASE (after ① and ②) = the 41st block's steps, with TWO changes:** step 2's migration set is now **2→5** (dry-run step 5 on a prod copy too — prod has no roleplay block and no agents dir, so it must be a no-op); step 5's phone card gains the mouth-wait arms below. The owner-questions paragraph of the 41st block is RESOLVED (all three ruled this session).

**▶▶ THE OWNER'S PHONE CARD — the mouth waits (dev :8443, RELOAD first — dist rebuilt; `voice.live.debug` is still ON):** 1. Ask something, then keep TALKING while it thinks: the reply must NOT start over you and must NOT vanish — it starts once you stop, and your addition is in the thread (a steer or the next turn). 2. Ask something and let the TV / a next-room voice talk during `thinking`: the reply starts within ~1 s of being ready (the noise verdict), "too quiet" may show for the noise's own final — that is correct. 3. Headphones, talk over an audible reply with interruption OFF: still queues; flip interruption ON: still cuts. 4. Mute mid-sentence, unmute, ask again: the next reply must not hang (the A2 strand). If anything hangs on `thinking` with the flags up, "check the trail" (`ls -t ~/.ctrl-b-dev/calls | head -1`).

**Ops notes:** the Emma lane = `hermes -p emma -m gpt-5.6-sol-900k --reasoning high --ignore-rules -t file,terminal -z …` (the owner's word: "double check with Emma, as always"; three rounds this session, ~10 min each, all self-contained); the session's briefs/designs/reviews are copied to `~/.cache/tmp/ctrlb-session42/` (disk). The build lane and the main seat edited DECISIONS.md concurrently without a clobber (both re-read before writing) — still, prefer disjoint files per lane. Dev units RUNNING. The R90 clones were deleted by the research lane. Usage limit hit mid-session — the code round and the S8 build were deliberately left for the clean session.

## Prior state (2026-09-26, FORTY-FIRST session — **THE PRE-RELEASE AUDIT ROUND, TWICE OVER: two Opus 5.5 end-to-end audits (R86 the live call · R87 the roleplay/card shape vs SillyTavern) → rulings → pinned-Opus fix waves → blind code rounds (Maya · fresh Opus) → confirms → COMMITTED `092130b` · `9c28e25`; THEN, on the owner's word, the SAME two lanes audited again by EMMA (hermes `emma` profile, `gpt-5.6-sol-900k`, `-z --ignore-rules`): R88 (live call, 2 MED) · R89 (roleplay, 4 MED + 1 LOW) → rulings → the same two Opus lanes (context intact) built the waves → Emma's SELF-CONTAINED confirm rounds (both SHIP after one tightening each) → gate 6/6 → COMMITTED `5aa59e0` (R88 wave) · `a9959fe` (R89 wave) + this docs commit. PROD TTS FLIPPED to PocketTTS (config backed up first). The prod config's 2→4 migration DRY-RUN on a copy is clean. e2e sweep 383/383 after one locator hardening (`2165f2a`). ▶▶ NEXT SESSION = THE RELEASE, in a CLEAN session (owner's word) — the exact steps below.**)

**① What the audits found and what changed (the dossiers are the record: `docs/research/R86` · `R87` (Opus) · `R88` · `R89` (Emma); the rulings = the D70/D71 amendments).** *R86 (live call):* no HIGH, 4 MED — the §4.2 iron rule killed + CANCELLED a reply on a raw VAD event (noise ⇒ a lost answer with "too quiet" on screen) → now EVIDENCE-GATED on the D74 gate's accrual; an `upstream_error` stranded `waitingFinal`; an all-failed chunked synthesis in a call was silent (exactly the PocketTTS-flip failure mode); a route flip refused `busy` by its own old leg ended the call. Plus LOW: audio before `ready`, the idle clock vs a talking reply, the "ships OFF" doc rot, and a frozen phone holding the single slot for 30 min → the new `voice.live.uplink_idle_s` relay reaper (15 s, server knob). *R87 (roleplay):* 2 HIGH on the owner's own library — `{{original}}` at offset 0 (14/36 system-prompt cards) put "You are ctrl-b" as the character's Voice and the persona under Duties → `{{original}}` = the configured `inference.system_prompt` else nothing, Duties always its own section; macros were case-sensitive (`{{Char}}`/`{{User}}` literal in 10 cards + 81 book hits) → ASCII case fold. MED: the stripped scripts still rode inside the served avatar PNG → `chara`/`ccv3` chunks removed, image ends at IEND, ST's `regex_scripts` denylisted; card-embedded books read the lossy V2 mirror not ST's `extensions.*` → extensions-first unless the entry is already ours; the `card` stash was NOT the restore point it claimed → **`agents/<slug>/card.json`** (0600, the field's `{spec, spec_version, data}` envelope) is the one kept copy and `AgentDef.card` is GONE (FE field/hint/fixtures with it). Deferred R87 findings = **ISS-21..28** (two need the owner: ISS-21 the lorebook framing, ISS-25 the per-agent persona object). **The Emma round (R88/R89) on top:** *live call* — the LC-1 exemption had treated a still-OPEN utterance's partial accrual as a quiet verdict (the auto hold could then close the ear on the rest of the owner's sentence) → the exemption is for CLOSED segments only (an open one keeps the conservative kill; continuous noise during `thinking` still kills — the deferred-mouth design is the owner's question); the idle clock now waits out the owner's own speech + a pending final; `/voice/status.live_call` stopped delivering `max_session_s` + `vad_threshold` (never read; a reader-parity test scoped to the eleven consumer files pins it); the example config's live block is pinned as a full inventory of `LiveCfg` (five keys were missing); plan §5.1/§5.3/§4.1 + D71's base line reconciled. *Roleplay* — a card with NO persona fields imported as ctrl-b → the registry prompt `card_blank_soul` (`You are {{char}}.`); concurrent imports raced on the slug → route-level locks held by `_import_locked` until the worker THREAD ends even on a cancelled request (`CoreMemory._guarded`'s shielded shape — Emma's confirm found the plain `async with` released early); lorebook KEYS join the unrendered-macro report; `card.json` keeps envelope-level unknowns at the envelope (`{spec, spec_version, <unknowns>, data}`, strip pointers `/data/…`); the import report's `mapped` = what was ACTED ON (`nickname`, a CHARX icon, a landed book). Behaviour notes: an off-spec card with `character_book`/`nickname`/`extensions` BESIDE `data` is no longer mapped from there (ST parity); `stripped_paths` read `/data/…` for V2/V3.

**② Prod, as it stands now (v1.7.7 still running):** `voice.tts` → `emma-pockettts/pocket-tts` (voice `ganyu`, `format: wav`, `chunk_format: wav` — set so v1.7.7 labels PocketTTS's WAV bytes honestly until the byte-sniff arrives with v1.7.8), fallbacks Kokoro → vault-alltalk; verified served-by PocketTTS. **Backup:** `~/.ctrl-b/config.yaml.20260926T090202Z.pre-pockettts`. Prod has NO `voice.live` block ⇒ the ear turns ON at v1.7.8. Dev: `voice.live.debug` still ON; the two imported agents keep a stale `card:` key in `agent.yaml` (inert extra — re-import them from their cards if you want `card.json` restore points); dist REBUILT this session.

**▶▶ THE RELEASE (clean session) = `deploy/linux/README.md` §Release end-to-end:**
1. `git status` clean, tip pushed, **CI green on the tip** (`gh run list --branch main --limit 1`).
2. **CONFIG BACKUP FIRST** (a second one, at release time): `cp ~/.ctrl-b/config.yaml ~/.ctrl-b/config.yaml.$(date -u +%Y%m%dT%H%M%SZ)` — v1.7.8 carries migrations 2→3 AND 3→4 (dry-run on a copy this session: only step 3 applies to prod, `wake.presence_device_ips` → `presence_devices`, values preserved; the loaded result had the ear ON + PocketTTS intact). Rollback = restore that backup, then v1.7.7.
3. Pre-deploy gate: `backend/.venv/bin/python tools/check.py --e2e` locally (this session: full 6/6 + e2e 383/383 at `2165f2a`; the two waves landed after the e2e run — re-run it).
4. Tag `v1.7.8` on the verified sha → push the tag → **wait for the CI release gate** → re-pin `~/apps/ctrl-b` → `install.sh prod` → `git describe --exact-match` + `/api/health` version `1.7.8`.
5. After: a first prod CALL (the ear is on in prod for the first time — Media route, loudspeaker, a few turns), the D75-amendment phone card (39th block) on PROD, then the owed arms (car clean route · TV · ISS-19).

**▶▶ OWNER QUESTIONS (not blocking the release):** (a) **R86 LC-1 / R88 E-1** — the iron rule now spares the reply ONLY for a noise segment that has already STOPPED and measured below `min_final_ms`. Anything still sounding when the reply's first chunk lands — the owner mid-sentence, OR continuous noise (a TV) during `thinking` — still kills and cancels the reply, whatever `barge_in` says. Two linked choices: should `barge_in: false` (the walkie-talkie default) let the reply play over the owner's REAL speech and queue their words for after it? And should the mouth be DEFERRED (held back) until a live segment is classifiable as speech or noise, instead of killing on it? (b) **ISS-21** the lorebook framing (instruction-chain books are introduced as "not instructions"). (c) **ISS-25** `AgentDef.user_name` → a per-agent persona object (cheapest before v1.7.8, a migration after).

**Ops notes:** CI on main went red THREE times (09-23 ×2, 09-26) on the same first-render Conf test timing out at vitest's 5 s default on ubuntu-latest → `testTimeout: 15_000` in `vitest.config.ts` (a hang guard, not a perf budget; `ccc955a`). The hermes lane: `pkill`/`kill $(pgrep -f 'hermes_cli.main -z')` also matches your OWN wrapper shell (exit 144, burned once this session) — filter on `venv/bin/python` before killing or waiting; two `-z` runs in parallel with distinct output files worked fine. The pre-push gate flaked ONCE this session on `frontend check-all` under the 6-parallel contention (passes alone; the second push went 6/6). The e2e Conf render arm was a real fragile locator (fixed `2165f2a`), not a flake. The Maya lane ran the R86 code round self-contained (`-z --ignore-rules`) in one go; the two Opus reviewers kept context for their confirm rounds (`SendMessage`) — the pattern to keep.

## Prior state (2026-09-26, FORTIETH session — **THE S4 CLOSE-OUT: PHASE 24 IS BUILT AND THE GATE IS CLOSED. `LiveCfg.enabled` default → `True` (ON since v1.7.8; the toggle stays), the two pinning tests re-aimed (the default is ON, an explicit `false` still wins), and the doc-truth sweep: LIVE_VOICE_PLAN header + §4.1's knob table REWRITTEN from `LiveCfg` (the as-built list — the draft's `barge_threshold`/0.9 rows are gone) + §5.1's sample + the S4 CLOSED gate record in §7 · TODO Phase 24 header + S4-D76 + S4 ticked · ROADMAP §C4 · CLAUDE.md's doc-map row · HTTPS_TAILSCALE now documents `:8443` → the DEV BACKEND `:5434` (built dist), with Vite as the HMR alternative. Gate 6/6. THIS COMMIT + THE 18 BEFORE IT PUSHED to origin this session (the owner's word: "commit and push"). ▶▶ NEXT = the prod TTS flip (PocketTTS) → v1.7.8 (the exact steps below, unchanged from the 39th session).**)

**① What closed.** The S4 gate's evidence is the D76 S3 round №2 (09-26, read from the D77 trail — all three arms PASS; the 39th block below has the numbers), so S4 closes on it: endpointing GOOD → v1 stands, architecture ② stays the recorded fallback, the Tier 0 `auto_stop_threshold` was never moved (0.01). **Prod effect at v1.7.8:** prod's `config.yaml` has NO `voice.live` block, so the new default turns the ear ON there the moment v1.7.8 is pinned — gated as always by `voice.enabled` and a configured realtime chain (prod's Speaches). Dev's config wrote `enabled: true` explicitly long ago; nothing moved there. **Owed, non-gating, riding the owner's regular use (recorded in the plan's S4 block + ISSUES):** the car on the CLEAN route (then `min_final_ms` 200 → 300 if home-side noise words persist) · the TV/other-room arm · ISS-19 (the Honor battery setting FIRST, then the lock arm, then the discard toast). ISS-13/14 parked won't-fix. The D75-amendment phone card (39th block) is also still unrun — it moves to PROD after the release.

**② Doc-truth notes.** §4.1's table is now a MIRROR of `LiveCfg` — when a knob is added or its default moves, the table moves with it (the same rule as QUALITY.md's counts: one source, the doc follows). `voice.live.debug` is STILL ON on dev (the readout + the trail) — turn it off in Conf when the rounds are done. Serve `:8443` → the dev backend `:5434`, `npm run build` after any FE change (the runbook now says so).

**▶▶ NEXT SESSION = THE RELEASE — the exact steps:**
- **Prod TTS flip (PocketTTS), BEFORE the release tag so v1.7.8 ships with it:** prod (`~/.ctrl-b/config.yaml`, service `ctrl-b-dashboard` :5433) has NO `emma-pockettts` provider — add it through the settings API on prod (`PUT /api/settings` with the dev block: `providers.emma-pockettts = {base_url: http://127.0.0.1:8890/v1, api_mode: openai, models: {pocket-tts: {voice: ganyu, format: wav}}}` + `voice.tts = {provider: emma-pockettts, model: pocket-tts, fallbacks: [{emma-speaches, Kokoro}, {vault-alltalk}]}`; the api_key on dev is a placeholder — check before copying); `pockettts.service` is a user unit already running on emma. Per-agent voices: the owner has NOT picked — `ganyu` is the interim default; the cloned library is in the `pockettts-tts-endpoint` memory.
- **v1.7.8 = `deploy/linux/README.md` §Release end-to-end.** ⚠ **CONFIG BACKUP FIRST** — the release carries config migrations 2→3 AND 3→4 (`cp ~/.ctrl-b/config.yaml ~/.ctrl-b/config.yaml.$(date -u +%Y%m%dT%H%M%SZ)`); rollback = restore that backup, then v1.7.7. Pre-tag: the e2e labels sweep (both waves' labels — the "Playwright `name:` is SUBSTRING match" gotcha) and `git describe --exact-match` + health after. The CI release gate must be green on the pushed tip before tagging.
- **After the release:** the D75-amendment phone card (39th block) on PROD, a first prod CALL (the ear is ON there for the first time), then the owed arms (car clean route · TV · ISS-19).

## Prior state (2026-09-26, THIRTY-NINTH session — **THE OWNER'S S3 ROUND №2 READ FROM THE D77 TRAIL: ALL THREE ARMS PASS, no voice fix needed. Then the owner's two agent asks became THE D75 AMENDMENT — the gallery card's "default" pill IS the default-agent control (three-state field) · the sticky pick PERSISTS · `/new` follows THE OWNER'S TANDEM RULE — designed → double-council-closed (Opus 5.5 ∥ Maya, design v1→v3.1) → BUILT by one pinned-Opus lane → blind code round (Maya SHIP-WITH-FIXES ∥ Opus SHIP-WITH-FIXES) → fix wave → confirm (Maya SHIP · Opus all resolved) → last polish wave → gate 6/6 → COMMITTED `820364b` → dist REBUILT. ⚠ 18 COMMITS UNPUSHED over origin `63da966` (incl. this handoff) — PUSH on the owner's word. ▶▶ NEXT SESSION = the S4 CLOSE-OUT → push → prod TTS flip → v1.7.8 (the exact steps below).**)

**① The S3 round №2 verdict (the trail did its job — `~/.ctrl-b-dev/calls/f5c4…` · `ef51…` · `74a4…`, 08:18–08:22).** *Loudspeaker, Media route:* every chunk probed HELD (leak −24 dBFS vs floor −31.5), the ear stayed shut, no self-transcription; a 40 ms "Mm-hmm" while waiting was dropped as too quiet (correct — 10 dB under the learned voice). *Call route, default mic:* the first turn accepted UNSEEDED at −8 dBFS peak; EC-on skips the probe by design; two residual-echo blips under the reply rejected by the 200 ms gate. *Headphones, Media route:* every chunk RELEASED, phase stayed listening, the over-talk ("Okay, okay") queued and sent at drain. Non-voice datapoints: the FIRST reply took 70 s = corsair's cold start (the turn's reasoning was 206 chars) — later turns 6–9 s; the swallowed `prefix_padding_ms` error on every leg is the documented Speaches quirk (the update still applies). **Nothing in the voice path changes from this round.** Still owed from the old card, none gating: the TV/other-room arm, the car arm, ISS-19 (the Honor battery setting + the lock-screen arm + the discard toast).

**② THE D75 AMENDMENT (record: DECISIONS D75, the AMENDMENT block after "RULED 2026-09-24: STICKY"; ISSUES ISS-20; commit `820364b`).** The owner's asks: "select one [agent] as the default in the library … the same space that is the default label could be used as the button itself" and "`/new` … keep the last used agent regardless". Opus's HIGH on v1 (a persisted pick makes a tapped default invisible on the device) → **the owner's TANDEM RULE**: `agent.default_agent` has THREE states — `""` NONE (deselected) · `"default"` the ROOT explicitly · a specialist. **A default SET ⇒ `/new` CLEARS the sticky pick (the new thread runs as the default); NONE set ⇒ `/new` KEEPS the agent the owner was talking to** (promoting the thread's pin when nothing is sticky). Built: `resolve_agent` short-circuits the root's slug; `/api/agents` gains `default_set` (= configured AND resolves — a dangling name presses nothing); the card's read-only badge became a sibling TOGGLE pill (`aria-pressed`, stable label `<title>: default agent`, quiet save, no pin, no navigation; translucent 76 % `--bg` veil, compact 3px/7px/9.5px, `background-origin: border-box` on the pressed fill — a gradient `--accent-fill` tiled under the transparent 1px border showed SOLID ENDS on gacha — and the pill joined gacha's seg-active gradient window); Conf's "Default agent" Seg gets "none" + saves IMMEDIATELY like the pill (two doors, one save style); `sessionAgent` → **`stickyAgent`** persisted under `ctrlb.chat` `{agent}` (`writeSticky` = the one persist seam); `startNewThread({keepAgent})` REQUIRED arg; `effectiveAgent` folds an unknown pick only against a LOADED roster (`undefined` = not landed); the root slug is always a valid pin (`lib/agentSlug.ts`, a leaf); ONE roster installer `installAgents(data, gen)` fed by `loadAgents()` AND the roster `queryFn`, each claiming `beginAgentsLoad()` before its fetch; the tools menu lists the ROOT row first + each specialist once (it used to list a specialist default twice and the root never), tags the resolved default ("root" on the root when it is not), and pins through `agentPin(name, threadAgent, defaultName)` — the one expression the gallery's Talk uses. **Recorded residuals (D75 ④–⑥):** `/new` inside one RTT of an explicitly opened character thread loses the promotion; two PWA tabs don't see each other's pick (no `storage` listener — ruled, single-user one-PWA); the `["agentlist"]`/`["agents"]` query split can disagree for one refetch (ISS-20). Auto-route (7e-g, OFF in both configs): with a default set `/new` starts unpinned so auto-route may pick a specialist; with none set it fires only after a bare `/agent`. Counts: BE 2,652 · FE 3,909/198.

**③ The cadence this session.** Design (`design.md` in the session scratchpad; the ruling of record = the D75 amendment) → blind Opus architecture ∥ Maya correctness [v1: Opus 1 HIGH · 3 MED · 5 LOW, Maya 4 MED] → the owner's tandem rule → v3 → confirm both (Opus 1 MED = the `/new` rule must read the roster QUERY, not the module copy; Maya B3 same gap + the auto-route caveat) → v3.1 → ONE pinned-Opus build lane (three waves on the SAME lane via SendMessage, context intact — the pattern to keep) → blind code round [Maya 2 MED · 1 LOW ∥ Opus 2 MED · 9 LOW] → fix wave (13 items) → confirm [Maya SHIP ∥ Opus all 11 resolved + 2 MED · 2 LOW] → last wave (root tag · pill legibility `--text-2` · `agentPin` dedup · 3 stale sentences) → full gate 6/6 → commit. The Maya lane ran FOUR self-contained rounds without a hitch.

**▶▶ THE OWNER'S PHONE CARD — eyeball the amendment (dev :8443, RELOAD first — dist rebuilt; the dev config ALREADY has Lynette as the configured default, so her pill is pressed):**
1. **Agents gallery:** the pills read translucent + compact, the pressed one shows the gradient with NO solid ends (gacha). Tap Lynette's pressed pill → it unpresses (none set); tap Seraphina's → pressed; tap the root card's → pressed (root explicit). Conf → Agent globals → the "Default agent" Seg follows each tap (none / default (root) / …) with no save bar.
2. **`/new` with a default SET:** talk to Seraphina (Talk), `/new` → the fresh thread answers as the DEFAULT; the backdrop + the tools menu's checked row agree.
3. **`/new` with NONE set:** deselect the default, talk to Seraphina, `/new` → still Seraphina. Kill the PWA, relaunch → still Seraphina (the pick persists).
4. **Tools menu (composer ⋯):** the root row is FIRST (tagged "root" while a specialist is the default, "default" when nothing is set), each specialist once, no duplicate.
5. Anything that reads wrong (labels, the pill's size over portraits on a light theme, the Conf wording).

**▶▶ NEXT SESSION = THE S4 CLOSE-OUT, THEN THE RELEASE — the exact steps (nothing here needs the phone):**
- **S4 close-out (one commit):** `backend/app/config.py:682` `LiveCfg.enabled: bool = False` → `True` (+ the docstring line "`enabled: false` ships until S4 passes" at ~674 → "ON since v1.7.8"; fix any test pinning the default) · `docs/LIVE_VOICE_PLAN.md` header (lines 1–12: S4 CLOSED 2026-09-2x, `enabled` ON) + §4.1's knob table rewritten to the D76 knobs (`vad_threshold` 0.6 · `silence_ms` 700 · `mic_hold` auto/on/off · `route` media/call · the six gate rows `floor_dbfs`/`noise_margin_db`/`voice_margin_db`/`min_dbfs`/`max_dbfs`/`playback_margin_db` · `min_final_ms` 200 · `trim_silence` · `trail_keep` — read `LiveCfg` for the exact list, the table must match the code) · `docs/TODO.md:1695` S4-D76 + `:1700` S4 ticked (the S3 round №2 verdict as the gate record) · `docs/HTTPS_TAILSCALE.md:74/86/118` → `:8443` fronts the DEV BACKEND `:5434` (built dist), not Vite (`tailscale serve --bg --https=8443 5434`) · ROADMAP §C4 pointer if it says "pending S4". Then the dev unit restart is NOT needed for the flip (`--reload`).
- **Push** the 18+ commits (owner's word already implied by "ship the feature complete" — CONFIRM once, then `git push`; the CI release gate must go green before tagging).
- **Prod TTS flip (PocketTTS), BEFORE the release tag so v1.7.8 ships with it:** prod (`~/.ctrl-b/config.yaml`, service `ctrl-b-dashboard` :5433) has NO `emma-pockettts` provider — add it through the settings API on prod (`PUT /api/settings` with the dev block: `providers.emma-pockettts = {base_url: http://127.0.0.1:8890/v1, api_mode: openai, models: {pocket-tts: {voice: ganyu, format: wav}}}` + `voice.tts = {provider: emma-pockettts, model: pocket-tts, fallbacks: [{emma-speaches, Kokoro}, {vault-alltalk}]}`; the api_key on dev is a placeholder — check before copying); `pockettts.service` is a user unit already running on emma. Per-agent voices: the owner has NOT picked — `ganyu` is the interim default; the cloned library is in the `pockettts-tts-endpoint` memory.
- **v1.7.8 = `deploy/linux/README.md` §Release end-to-end.** ⚠ **CONFIG BACKUP FIRST** — the release carries config migrations 2→3 AND 3→4 (`cp ~/.ctrl-b/config.yaml ~/.ctrl-b/config.yaml.$(date -u +%Y%m%dT%H%M%SZ)`); rollback = restore that backup, then v1.7.7. Pre-tag: the e2e labels sweep (both waves' labels — the "Playwright `name:` is SUBSTRING match" gotcha) and `git describe --exact-match` + health after.
- **After the release:** the owner's phone card above on PROD, then the owed arms (TV · car · ISS-19).

**Ops notes:** Serve `:8443` → the dev BACKEND `:5434` (built dist — `npm run build` after ANY FE change; DONE). The dev backend runs `--reload` (the new listing field is live: `default_set: true, default: lynette`). `voice.live.debug` is still ON on dev (the readout + the trail) — turn it off in Conf when the rounds are done. The builder-lane pattern that worked: ONE pinned-Opus lane, then fix waves by `SendMessage` to the SAME lane (context intact, three waves, no re-reading); Maya confirm rounds stay self-contained (`-z --ignore-rules` keeps no session).

## Prior state (2026-09-25, THIRTY-EIGHTH session — **THE OWNER'S S3 ROUND №1 DECODED (two verdicts, both traced to root cause), THE FIX WAVE + D77 THE CALL TRAIL designed → Maya-council-closed → BUILT (three pinned-Opus lanes) → main-seat audited → blind two-lens code round (Maya BE ∥ Opus FE) → both fix waves applied → gate 6/6 → dist REBUILT for the phone. Commits: `fc51d0c` (the TTS pad trim) · `eb1d93b` (D77 + the voice-level key) · this commit (this handoff + D76 addendum + D77 + plan/TODO/SECURITY_MODEL). ⚠ NOW 16 COMMITS UNPUSHED over origin `63da966` — PUSH on the owner's word. ▶▶ NEXT SESSION OPENS ON THE OWNER'S PHONE CARD (below) — S3 round №2, WITH THE TRAIL ON; the main seat reads the trail instead of the owner narrating.**)

**① What the owner reported and what it was.** *(a) Media route + `mic_hold: auto` on the loudspeaker: "the agent can hear itself."* MEASURED, not guessed: PocketTTS opens EVERY clip with a 10 ms −58 dBFS click and then 320–760 ms of −81 dBFS digital silence before the first sound (three sentences synthesized on dev: 760 · 320 · 640 ms). The leak probe judges the first 600 ms after `playing` — it heard pad, said "no leak", opened the ear, and the reply's real speech leaked into the open mic and transcribed itself. The probe's premise broke, not its logic. **Fix (S3a):** synthesized WAV clips are trimmed at the ONE TTS chokepoint (`core/audio.py::trim_wav_silence` in `VoiceClient.synthesize`, bytes-sniffed — PocketTTS answers WAV to any format ask; the media type now follows the bytes too), knob `voice.tts.trim_silence` (default ON, server-side). On the measured clips 810 · 270 · 590 ms removed; sound now starts at 40–50 ms — every sentence starts sooner everywhere, read-aloud included. `PROBE_MS` stays 600. *(b) Call route + the "default" Mic row: deaf until Speakerphone; the owner confirmed the "too quiet" note and escaped by dragging the Sensitivity marker down.* TRACED: the learned own-voice level was keyed by device alone; the same mic opens quieter under the Call route (comm-mode processing), so the Call capture inherited Media's loud level, `voice − 10 dB` sat above the owner's voice, and every final dropped. **Fix (S3b):** the key is `"<device>|ec=<all|on|off>"` from the GRANTED echo readback; legacy flat keys purged on write; the key shows on the debug readout (its own line). **Recorded, not built** (DECISIONS D76 addendum): the within-mode deadlock (a ≥10 dB posture change ⇒ nothing accepted ⇒ the level never lowers; the pin is the escape and relearns); a chunk that fires `playing` then stalls > 600 ms would still release (chunks are complete blobs — theoretical).

**② D77 — THE CALL TRAIL (S3c), from the owner's ask ("so you actually can see exactly what happened in a call").** With Conf → Voice · Live → **Call debug readout ON**, every call writes `~/.ctrl-b-dev/calls/<call_id>.jsonl` (dev; `$CTRLB_HOME/calls/` in general), BOTH halves: the relay (`leg_start` with the exact `session.update` · every `down` frame incl. the transcript text · `flush` · `stop` · `up_error` · `leg_end`) and the browser via `POST /api/voice/live/trail` (every reducer signal at the one `send()` chokepoint — `sig` with phase/note moves, `textLen` not text, the `final`'s pre-reduce hold snapshot, `genMove`, an `oversize` marker past 2 KB · a `final` line with the meter's peak/accrued ms + the floor it met · `capture` with the readback + voice key/seed + gate knobs · `probe` per verdict (max dBFS vs floor, released/held) · `sample` at 1 Hz = the moving numbers + phase). Identity = a client-minted UUID + the hook's leg counter on every line and in `start` (`call_id` + `leg`, both or neither); 404 when debug is off and the wire's `start` is byte-identical; retention `voice.live.trail_keep` (20, a Conf row "Trails kept"); files 0600 in a 0700 dir; the owner's words appear once, on the relay's transcript line (SECURITY_MODEL §2.11). **To read a call: `ls -t ~/.ctrl-b-dev/calls | head -1`, then `cat`/`jq`.** **The dev config's `voice.live.debug` was FLIPPED ON via the settings API at the end of this session** (the readout + the trail; the route answered 404 before, 422-on-empty after) — the owner sees the debug numbers on the call screen until they turn it off; the card assumes it stays on for the round.

**③ The cadence this session.** Design (`design.md`, kept in the session scratchpad; the ruling of record is DECISIONS D76-addendum + D77 and the plan's S3 block) → blind Maya design round BUILD WITH CHANGES [4 MED · 1 LOW: leg identity · the keepalive byte cap · one transcript copy · the RIFF walker's contract FOLDED; the stall class RECORDED] → three pinned-Opus lanes (S3a caught the click and added the two-frame onset rule; S3c added the 415 rail, streamed body cap, `leg_end` in `finally`) → main-seat audit → Maya BE code round SHIP WITH FIXES [5 MED, all fixed main-seat + 2 tests] ∥ Opus FE code round SHIP WITH FIXES [2 MED · 3 LOW, all fixed by a pinned-Opus wave] → gate 6/6 (BE 2,651 · FE 3,890/198) → `npm run build`. The stash gotcha: an Opus lane ran `git stash`/`pop` on the shared tree mid-wave — nothing was lost, but tell lanes NOT to.

**▶▶ THE OWNER'S PHONE CARD — D76 S3 round №2 (dev :8443, RELOAD the page first — the dist is rebuilt; the SW is prompt-mode so the app's own update prompt may show):**
0. **Conf → Voice · Live → Call debug readout is already ON** (flipped for you; it now also writes the trail — leave it on for the whole round). Conf → Voice · TTS → check "Trim silence" reads ON (it should by default).
1. **Loudspeaker at home, Media route:** a call, a few turns. Expect: replies start noticeably sooner; the debug line shows `probe #n … held` on chunks; NO reply words come back as your message. If any do, just note the time — the trail has it.
2. **Call route, the "default" Mic row:** hang up, switch Sound to Call, Mic to default, call again, speak on the FIRST turn at your normal distance. Expect it heard (the Call key starts unseeded). If "too quiet" shows: do NOT pin — say roughly how far the phone was; the trail has the floor and the peak.
3. **Headphones, Media route:** expect `released` on the debug line and duplex (your words under the reply queue or cut it per the interruption toggle).
4. **Anything else from the old card** (TV, car, ISS-19 lock screen) as time allows.
5. After the round, one message: "check the trail" (+ which arm felt wrong). No narration needed — that is the point of D77.

**▶▶ AFTER THE CARD:** the main seat decodes the trail(s) → fixes if any → the S4 §4.1 close-out (`LiveCfg.enabled` → ON, Phase 24 S4 checked, §4.1's knob table rewritten, TODO S4-D76 ticked) → prod TTS flip (PocketTTS) → v1.7.8 (**config backup FIRST — migrations 2→3 AND 3→4**; `trim_silence`/`trail_keep` are additive, no migration).

**Ops notes:** Serve `:8443` → the dev BACKEND `:5434` (built dist — `npm run build` after ANY FE change; DONE this session). The dev backend runs `--reload`, so the new route/knobs are live; `trail_keep`/`trim_silence` read their defaults (the dev config never wrote them). The Maya lane ran two self-contained rounds (`-z --ignore-rules`) without a hitch. HTTPS_TAILSCALE.md still documents :8443 as the Vite door — a doc-truth item for the close-out (carried).

## Prior state (2026-09-25, THIRTY-SEVENTH session — **THE D76 WAVE: from the owner's two phone rounds to a RULED, council-closed design and FOUR BUILT SLICES in one sitting. The round №2 answers (app switch HOLDS · the mic-source flip mid-reply moved NOTHING ⇒ no route-only lever, R81 §5 closed) · the ISS-18 notice moved to the Sound picker's static footer (`272ce9a`) · Serve :8443 re-pointed at the DEV BACKEND (built dist, prod-shaped, no HMR) · FOUR research dossiers R82–R85 (`c21022c`) · D76 RULED + Maya ×3 design rounds (`77bd6f5`) · S0a `5e7b668`+`6eeabf1` · S0b `77fd2c2`+`dbf3978` · S1 `9de6aef`+`40569a2`+`61ec24b` · S2 `429c3b0` — every slice pinned-Opus-built, main-seat audited, blind-Maya code-rounded, fix-waved. Gate 6/6 at every tip; BE 2,579 · FE 3,862/197. ⚠ 12 COMMITS UNPUSHED over origin `63da966` (+ this handoff) — PUSH on the owner's word. ▶▶ NEXT SESSION OPENS ON THE OWNER'S PHONE CARD (below) = D76 S3, the gate.**)

**① What the owner has on dev right now (dev :5434 behind `https://emma.lobster-vector.ts.net:8443`, the BUILT bundle — reload the page; the SW is prompt-mode so the app's own update prompt may show first).** The deck = **Sound · Mic · Sensitivity**. Sound has TWO rows: **Media** (default — "clear audio · follows Bluetooth like music, phone speaker otherwise") and **Call** ("phone-call mode · echo-cancelled · hands-free mic"); the footer says a change mid-reply starts with the next reply. The Speech slider is GONE (Silero 0.6 in Conf, bounded 0.5–0.8). **Sensitivity** opens a live vertical meter: the bar is the mic level, the marker the automatic floor (noise floor + 10 dB, or your own learned level − 10 dB, whichever is higher; permissive for the first 5 s, never above −45 dBFS until settled); drag the column to PIN a floor for this call (bottom = more sensitive), tap the caption to go back to Auto. Conf → Voice · Live: the route Seg (media/call), **"Mic off while it speaks"** (auto/on/off — `auto` = the LEAK PROBE: each reply chunk starts held for 600 ms while the mic listens for the reply leaking in; no leak ⇒ the ear opens for the rest of the chunk; headphones therefore get duplex, the loudspeaker/car stay held), the six gate rows (floor ceiling · noise/voice/interrupt margins · the two bounds). A final dropped as too quiet now BEEPS (120 ms, 440 Hz) besides the note. The owner's dev config: `route: media` (flipped via the API on their word), `mic_hold` unset ⇒ auto, migrated to schema **4** (backups in `~/.ctrl-b-dev/backups/`). Debug readout (Conf → Call debug readout) now speaks dBFS and shows `probe #idx max floor released|held` — **S3's evidence line**.

**② The design in one breath (record: LIVE_VOICE_PLAN §7 "THE D76 WAVE" — A–H + the council + the per-slice as-built; DECISIONS D76; research R82–R85).** The car's "had to shout" and the home's false short turns were ONE fault: an absolute linear floor (0.06 = −24.4 dBFS, 10–35 dB stricter than every shipped default) calibrated for barge-in on the phone mic, plus a Silero END threshold at the cliff (0.9 − 0.15); and "I have to pick headphones by hand" was a NAMING fault (Speaker = comm mode, which ignores media-only Bluetooth; the media path already follows BT like music). Verified at source: Chrome Android has NO adaptive AGC (fixed +6 dB); Silero is loudness-blind and the start threshold is not the noise lever; the ear-hold was `track.enabled=false` (a held mic hears nothing — S0b changed it to uplink silence substitution so the probe can listen); `playbackStarted` is a per-REPLY status edge (S2 added the per-chunk `setCallChunkStart`).

**▶▶ THE OWNER'S PHONE CARD — D76 S3 (the gate; dev :8443, reload first; turn the Call debug readout ON in Conf for the numbers):**
1. **Headphones, Media route (the whole point):** connect, start a call, speak. Expect the reply IN THE HEADPHONES without touching anything. Watch the debug line on the first reply: `probe #0 … released` within ~0.6 s, and the phase should stay LISTENING under the reply (duplex). Talk over the reply: with hands-free interruption OFF your words queue and send when it finishes; with it ON (Conf) they should cut it. Report: did it release? any of your own reply's words come back as your turn? any deafness at chunk starts you noticed?
2. **Loudspeaker at home, Media route:** same call, headphones off. Expect `probe … held` on every chunk (the mic hears the reply), the mic pause as before, NO self-transcription. If a reply's words ever appear as your message, that is the probe missing — note the `max`/`floor` numbers on the debug line.
3. **The TV / other-room voice test:** with the TV on, sit quiet for ~10 s (the meter's marker should settle above the TV's level after a couple of turns from you), then let the TV talk. Expect nothing sent (or "too quiet" + the beep). Then speak normally: expect your turn taken. Report the marker position vs your voice on the meter.
4. **The car, Media route (A2DP):** reply through the car speakers, the PHONE mic listening. Expect: no "had to shout" (the floor is relative now — in engine noise it rises with the room, never above −45 dBFS while learning), `probe … held` on chunks (the car speakers leak into the phone mic), the beep if a turn IS dropped. If turns still drop: drag the Sensitivity marker DOWN (more sensitive) for this call and tell me the dB you landed on — that is the S3 number the design needs. Also try the **Call** row in the car once (the car's own mic, phone-call sound) and say which you prefer.
5. **ISS-19 (still owed from the last card):** Settings → Battery → App launch → Chrome → Manage manually (all ON); then a call · lock the screen 3 min · unlock · speak. Does it hear you? On return, does the "the browser discarded this page…" toast show?
6. **Anything that reads wrong:** the row names (Media/Call), the Conf descriptions, the meter's polarity (bottom = more sensitive), the beep's loudness.

**▶▶ AFTER THE CARD:** the fixes it picks (PROBE_MS / margins / the `floor_dbfs` ceiling are the tunables; the text self-echo backstop is the recorded fallback if the probe misses) → the S4 §4.1 close-out (`LiveCfg.enabled` default → ON, Phase 24 S4 checked, §4.1's knob table rewritten to the new knobs, TODO S4-D76 ticked) → prod TTS flip → v1.7.8 (**config backup FIRST — the release now carries migrations 2→3 AND 3→4**; rollback = the backup, then v1.7.7).

**Ops notes for the next session:** Serve `:8443` fronts the dev BACKEND `:5434` (not Vite) — after ANY FE change run `cd frontend && npm run build` or the phone sees stale code; `tailscale serve --bg --https=8443 5173` puts HMR back. The dev-web (Vite) unit still runs, unused. The app REFUSES to start on a stale config schema (exit 78) — `CTRLB_HOME=~/.ctrl-b-dev backend/.venv/bin/python -m app.config_migration --apply` from `backend/`, then `systemctl --user start ctrl-b-dashboard-dev`. The Maya lane ran SIX self-contained rounds this session without a hitch (`-z --ignore-rules`; confirm rounds must paste the prior review). HTTPS_TAILSCALE.md still documents :8443 as the Vite door — a doc-truth item for the close-out.

## Prior state (2026-09-24 night, THIRTY-SIXTH session, THE LATE SITTING — **THE OWNER'S S4 ROUND №1 DECODED + THE ISS-18 SLICE BUILT, DOUBLE-REVIEWED, CONFIRMED, PUSHED. The round: car (EC-on route → the car speakers, correct; the car mic + the quiet-final gate = "had to shout") · headphones (the Mic rows = `default · Speakerphone · Headset earpiece` — R77's no-HFP-row premise STANDS; a mid-reply flip stayed on the phone speaker until a later reply = ISS-18) · lock screen (deaf in ~2 s, no reply audible = ISS-19, device-setting probe first) · noise → short false words (the same gate, other side) · ISS-13/14 PARKED won't-fix (owner scope ruling) · the reload-on-every-return on dev = VITE's HMR client, not the app. **Built:** R81 (Chromium source, verified — the output stream's tag is fixed at `src` assignment; pooled; `pause()` = 15 s, unload + 5 s = fresh) → the fresh-stream slice (`markStreamRetag` · the in-call finish UNLOADS · the first-`src` hold, 5500 ms platform constant) + the boot-time discard notice (`document.wasDiscarded` → sticky toast); blind Maya DO NOT SHIP [3 MED] + Opus SHIP WITH CHANGES [1 MED · 3 LOW · S1] → both fix waves built → both CONFIRMED. FE 3,780/194, gate 6/6. ▶▶ NEXT SESSION OPENS ON THE OWNER'S PHONE CARD (below).**)
...
**▶▶ NEXT SESSION:** ~~① push~~ DONE ~~② ISS-17~~ DONE ~~③ S1~~ RULED session-scoped ④ ~~the rest of the 35th session's eyeball list~~ CONFIRMED by the owner (the who-line · the tools faces · ganyu · the Karpathy row) — and **the "6-arm e2e flake" was NOT a flake: the 2026-09-21 every-ConfGroup-starts-collapsed ruling (`d5a787e`) never touched the e2e specs, and 7 arms × 2 projects (flows ×2 · a11y gacha-Conf · conf.spec Inference/STT/TTS/Embeddings · layout ×3) still assumed expanded groups. Only the release-tag CI runs Playwright, so it would have bitten at the v1.7.8 tag. FIXED: each single-group arm opens its group by `.conftitle` (conf.spec's own idiom); `openAllConfGroups` in `e2e/fixtures.ts` (by id — `.all()` nth-locators re-resolve against a shrinking set) for the two whole-tab arms; the a11y Conf rows wait for "chat backend" (the Inference header caption; `getByText("Inference").first()` hit a hidden provider row). Full suite 381/381.** ⑤ the standing D73/D74 probes → the S4 §4.1 close-out → prod TTS flip +
v1.7.8.
