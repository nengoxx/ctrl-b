...
**▶▶ OWNER QUESTIONS (not blocking the release):** (a) **R86 LC-1 interplay** — the iron rule now spares NOISE, but the owner's REAL speech at reply start still kills + cancels the turn whatever `barge_in` says; should `barge_in: false` (the walkie-talkie default) let the reply play and queue the words instead? One condition on one reducer line if yes. (b) **ISS-21** the lorebook framing (instruction-chain books are introduced as "not instructions"). (c) **ISS-25** `AgentDef.user_name` → a per-agent persona object (cheapest before v1.7.8, a migration after).

**Ops notes:** the pre-push gate flaked ONCE this session on `frontend check-all` under the 6-parallel contention (passes alone; the second push went 6/6). The e2e Conf render arm was a real fragile locator (fixed `2165f2a`), not a flake. The Maya lane ran the R86 code round self-contained (`-z --ignore-rules`) in one go; the two Opus reviewers kept context for their confirm rounds (`SendMessage`) — the pattern to keep.
