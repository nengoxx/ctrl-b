...
**Owner decisions — RESOLVED same day (owner ruling 2026-07-07):**

| # | Sev | Item | Resolution |
|---|---|---|---|
| **QH-10** | MED | **No conftest-level test isolation.** Every app-booting test file hand-rolled `_client()` env isolation; no `backend/tests/conftest.py`, so one forgetful future test boots against the repo root (proven 2026-07-07: root `memories/` git-init artifacts). | **Built** (owner confirmed after scope review — pytest-only, zero production effect): `backend/tests/conftest.py` autouse fixture points `CTRLB_HOME` at a per-test temp dir and clears inherited `CTRLB_CONFIG`/`CTRLB_DB`; explicit per-test envs still win; 254/254 green under it. |
| **QH-11** | LOW-MED | `TailscaleCfg.target_port` defaulted to **5173** (dev Vite) — the wrong safe-default direction (SYS-4 rider); the in-app `serve_https` action follows the config default. | **Flipped to 5433** (backend-served prod SPA; owner decision). Per-box override already existed at both layers and is now documented: `tailscale.target_port` in config.yaml + `CTRLB_TAILSCALE__TARGET_PORT` in `.env` (`_apply_env_overrides`); commented `tailscale:` block added to config.example.yaml; D-entry amended. |
