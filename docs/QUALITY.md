...
| **BE tests** | **pytest** (2,189 collected 2026-09-04 post-D2-C review fixes, temp-config safe — `conftest.py`: a module-level env guard that runs before any test module imports, plus the per-test autouse fixture; QH-10 + UPDATE_PLAN §3.7) | backend logic | ✅ |
