...
| **BE tests** | **pytest** (2,315 collected 2026-09-06 post-D70 S2 + its fix wave (card import), temp-config safe — `conftest.py`: a module-level env guard that runs before any test module imports, plus the per-test autouse fixture; QH-10 + UPDATE_PLAN §3.7) | backend logic | ✅ |
