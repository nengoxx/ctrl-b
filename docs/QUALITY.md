...
| **BE tests** | **pytest** (2,251 collected 2026-09-06 post-D70 S0+S1 + their Emma fix waves, temp-config safe — `conftest.py`: a module-level env guard that runs before any test module imports, plus the per-test autouse fixture; QH-10 + UPDATE_PLAN §3.7) | backend logic | ✅ |
