...
| **FE unit tests** | **Vitest** (D21 — 2,906 across 168 files, 2026-09-04 post-D2-C review fixes) | logic regressions | ✅ |
...
| **BE tests** | **pytest** (2,357 collected 2026-09-06 post-D70 S3 (the lorebook subsystem: scan · budget · placement · CRUD · book import · the card hook), temp-config safe — `conftest.py`: a module-level env guard that runs before any test module imports, plus the per-test autouse fixture; QH-10 + UPDATE_PLAN §3.7) | backend logic | ✅ |
