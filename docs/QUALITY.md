...
| **FE unit tests** | **Vitest** (D21 — 2,980 across 174 files, 2026-09-07 post-D70 §8.4a satellite placement: the pure `composeLayout` + its healing · the parameterized never-in-two-places guard (DefaultRoot + Conf across conf/button/tab, malformed, minimal, deep boot, live flip) · the placement-flip scroll-cache clear · the Conf tail numbering across every hosted combination · gacha's キャラ at five columns) | logic regressions | ✅ |
