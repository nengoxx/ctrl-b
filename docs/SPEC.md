...
| P2 | **Agent chat** | Streaming tool-loop chat (SSE), the full action registry as OpenAI tools, confirm-gated risky calls with suspend/resume, task plans (TodoWrite-style + clickable panel), clarifying questions, context compaction w/ selectable summarizer, per-message local/cloud switch, markdown replies, **composer attachments** (staged raw-body uploads → durable per-thread files → `read_attachment` re-reads + the per-hop no-vision strip, D68) | ✅ |
| P3 | **Agent platform** | File-discovered **skills** (SKILL.md; user- & selector-invoked, toolset narrowing), folder-discovered **agents** (agent.yaml + SOUL.md persona; `/agent` switch + optional auto-routing), headless **subagents** (bounded parallel, privilege-clamped), **memory** — tier 1 (MEMORY/USER/STATE stores, cap headers, propose-or-auto-write, git-versioned) + the opt-in tier-2 **Core Memory** corpus (shared markdown index + on-demand topic reads via `core_memory`, D57), **session search** (FTS5), loop-discipline guards, self-managed skills (`skill_manage`), and **characters-are-agents** — the AgentDef expansion + universal Voice/Duties assembly, V2/V3 card import, the roleplay-independent **lorebook** subsystem, the agent gallery + three-state backdrop (D70) | ✅ |
...
| P5 | **Voice** | Push-to-talk dictation (STT proxy, 5-state mic + the dual-mode hold/lock gesture), read-aloud TTS (per-bubble + auto-TTS, docked mini-player, blob cache) with **chunked read-along** synthesis (D63), primary→fallback failover per service, capability-probed UI, and **live voice / "call mode"** — the Speaches-realtime ear behind the app's one WebSocket relay, client-submitted turns, energy-gated barge-in, plus **phrase-streaming dictation** off the same ear (D71; `voice.live.enabled` ships ON since v1.7.8 — the S4 gate closed 2026-09-26) | ✅ |
| P6 | **Config & ops** | Whole config editable in-app (masked secrets, comment-preserving writes, hot-apply), Tailscale Serve HTTPS toggle, live events feed + audit trail, guarded `!` shell escape hatch (opt-in), PWA install/update, the **media manager** — typed raw-body writes + per-destination art LIBRARIES, crop, focal point, drag reorder (D65) | ✅ |
...
| Core Memory (D57, `core=False` — an allowlist may exclude it) | `core_memory` (`read`/`search`/`create`/`update`/`remove`/`delete` over the tier-2 corpus; hidden from the schema while `memory.longterm.backend` is null; D60: `read`/`search` are exempt from `max_calls_per_tool` and bounded by the recall budget instead, and `delete` takes `superseded_by`/`reason` + archives into `.archive/`; D64: `read` PAGES via `offset`/`limit` with a facts-only PARTIAL marker, and `delete` carries NO token — the tool mints it from the turn's own read coverage, fail-closed) | plain | decorator |
| Attachments (D68, `category="builtin"`, **`core=True`** — the §4.2 injection marker NAMES this call, so it may never be missing from a turn) | `read_attachment` (LOW, `read_only`, `ui_exposed=False`): no `name` → the manifest of what this thread holds; a `name` → that file's text, PAGED by `offset`/`limit` on the D64 contract. Images are never read here. Confined to the CURRENT thread through the server-owned `InvocationContext.thread_id` — **fail-closed when absent**, never a model-supplied argument | plain | decorator |
...
Per-tool **overrides** (`tool_overrides{name: {description, agent_mode, approvals, max_calls}}`, D22 + D44 + D60) overlay the live
...
|---|---|---|
| `server` | bind/port/poll cadence/debug **+ `trusted_hosts[]`** (R73/D72 ⑥ — the `Host` allowlist; **non-empty ⇒ `TrustedHostMiddleware` mounts at app construction, on `websocket` scopes as well as `http`; ships EMPTY = not mounted**, because a mounted empty allowlist 400s every route including the Conf UI — SECURITY_MODEL §2.9) | poll live; host/port/debug → restart-flagged; `trusted_hosts` is read at app construction |
...
| `prompts{}` | per-prompt overrides keyed by the registry id (Phase 18/D56 — the `REGISTRY` dict in `services/agent/prompts.py` IS the id list; it grows with every feature that speaks, so it is never enumerated here. D57 added the core-memory + consolidation ids, D60 `consolidation_dryrun`, D70 the Voice/Duties + persona + lorebook headings) | live |
| `voice` / `searxng` / `embeddings` / `open_terminal` | endpoints + per-op risk; **`voice.stt` carries the R51 Tier-0 mic policy — `auto_stop` (default **off**) · `auto_stop_silence_s` (0.5–30, default 3.0) · `auto_stop_threshold` (0.001–0.5, default 0.01) — a CLIENT policy delivered by `GET /voice/status` as `stt_auto_stop`, beside `stt_auto_send`**; **`voice.tts` also carries the D63 chunk policy — `chunking` (off\|paragraph\|sentence, default `sentence`) · `chunk_format` (`opus`) · `chunk_min_words`/`chunk_min_chars`/`chunk_max_chars`/`chunk_lookahead` — a CLIENT policy delivered by `GET /voice/status`, load-validated `chunk_min_chars <= chunk_max_chars <= max_text_chars` and lookahead 1..4**; and **`voice.live` (`LiveCfg`, D71) carries the whole live-voice leg** — the relay's provider/model/fallbacks on the house shape (blank provider rides `voice.stt`'s section), `enabled` (the CALL, ON since v1.7.8) and `dictation` (the streaming mic, ships OFF) — **the WS route and the `live_ear` bit admit on `enabled OR dictation`** — plus `allowed_origins[]` (the Origin rail's exact-string escape hatch), `max_sessions`, `uplink_idle_s` (the relay's frozen-page reaper, a server knob — R86), and the bounded client knobs delivered by `GET /voice/status` (`call_backlog_ms` · `tail_wait_ms` · `dictation_idle_s` · `dictation_max_s` · VAD/barge thresholds) | rebuild-on-change (the chunk policy is re-read by the PWA when the voice-status query invalidates) |
...
| `wake` | fleet wake automation: the D2-B connect cooldown + BOTH presence sources' tunables (D2-A/D50 + D2-C) — **`presence_devices[]`**, one object per watched device (`name` · `tailnet_ip`/`lan_ip`, ≥1 required · optional `lan_offline_after_s`, default 900), which config-migration **step 3** folded from the pre-D2-C `presence_device_ips` (**`config_version` 2 → 3**); the tailnet `presence_offline_after_s`; the LAN probe's `lan_probe_count`/`lan_probe_timeout_s`/`lan_health_ip` (the health gate — a dead local link must not read as every device absent); and `quiet_hours: {start, end}\|null`, which suppresses **LAN** arrivals only. Participation stays per-host (`wake_on_presence`) | live |
...
| `media{}` | owner media state keyed by **namespace** (mirrors `MEDIA_NAMESPACES`, incl. `kit`, which no theme owns — D52/G5 + D53); absent key = every consumer stays on its bundled art. **D65's fold is BUILT (S1)**: the namespace blocks live under `media.namespaces.<ns>` beside `media.write.max_bytes` (15 MB, `gt=0`) and each role's list is **`files: [{name\|bundled, key?, hidden?, focal?}]`** — one ordered list of per-item objects, identity a validator-enforced discriminated union unique per role (extend-don't-migrate); config-migration **step 2** folded it and deleted the old keys (**`config_version` 1 → 2** — the DB `schema_version` is untouched; MEDIA_MANAGER_PLAN §2.2) | live |
| `attachments{}` | composer attachments (D68) — the store + model-feed tunables in ONE block, grown by optional fields per slice: `max_files_per_message` (the ceiling the chat POST refuses past — on the CLAIM, since staging is id-addressed and belongs to no message yet), `max_file_mb` (streamed, counted as it arrives — never `Content-Length`, which is a claim), `staging_orphan_hours` (deliberately ONE number: both the boot sweep's cutoff AND the claim's freshness test), plus the S2 feed knobs (`max_inline_chars` = both the injected slice and `read_attachment`'s page size, `max_images_per_request`, the PDF bounds). A **sibling of `media`, not a key inside it** — they share a persist pipeline but not a purpose | live |
| `roleplay{}` | character agents (D70) — the GLOBAL half only: the UI-visibility switch, the import-time minimal toolset, the owner's own persona. A **sibling of `agent`, not a key inside it**: the per-agent half is flat fields on `AgentDef` | live |
| `lorebooks{}` | the lorebook subsystem (D70 §6) — `books` bindings + `max_import_bytes` + the budget/scan knobs. **Its own top-level section rather than a key under `roleplay`**, because books are roleplay-INDEPENDENT: any agent may carry one and the manager is visible with the character UI switched off (ruling 9). The books themselves are one YAML per file under `$CTRLB_HOME/lorebooks/`, discovered by scanning | live (loaded fresh per turn, so a save applies immediately) |
...
│   ├── tests/               # conftest + decision-pinned suites (test_*_d26 …) + drift guards (counts: QUALITY.md)
...
| Lint/type ratchets | ruff **E/F/I/ASYNC/B** (SYS-16's `+ASYNC/B` ✅ landed) · pyright `basic` · the deferred eslint warns = the F13 React-Compiler backlog (count: QUALITY.md, its only home) | ▹ pyright `strict` ratchet · ▹ `@eslint-react` |
