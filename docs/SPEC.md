...
| `media{}` | owner media state keyed by **namespace** (mirrors `MEDIA_NAMESPACES`, incl. `kit`, which no theme owns — D52/G5 + D53); absent key = every consumer stays on its bundled art. **▹ D65 folds the shape at S1** (ruled, not yet built): the namespace blocks move under `media.namespaces.<ns>` (making room for `media.write.max_bytes`, 15 MB) and each role's `order: [names]` becomes **`files: [{name\|bundled, key?, hidden?, focal?}]`** — one ordered list of per-item objects (extend-don't-migrate); a config-migration step folds it and deletes the old keys (**`config_version` 1 → 2** — the DB `schema_version` is untouched; MEDIA_MANAGER_PLAN §2.2) | live |
