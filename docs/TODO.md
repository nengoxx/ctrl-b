...
- [x] G5 media/roster serving — code + reviews ✅ COMPLETE 2026-08-05 (as-built §7.6; hardened mount + index, gallery, SW caching, degrade-never-brick health, reel-pool pin ruling; both §7.5 carries done) — stack PUSHED 2026-08-05 (owner word); **owner FILE-DROP + phone GALLERY round DEFERRED by the owner ("another time") — still owed before v1.5.0 ships**
- [x] **M1a–M3 media namespaces v2 (D53, `MEDIA_PLAN.md` §8 — BEFORE G6, owner-ruled):** ✅ all four children closed 2026-08-05, D53 FEATURE-COMPLETE *(parent box caught up 2026-08-12; the owner media-gallery round stays parked in HANDOFF)*
  ...
  - [x] M3 kit services — ✅ CLOSED 2026-08-05 (`d70d395`+`bbbb859`+`6dcb7a1`+R1 `8941676`+`06b6f7f`; `keyFor` (blank-kind→name, writer's rule) · ONE `ServiceIcon` w/ (url,revision) latch + `key={identity}` remount (the late-old-error race) · five verified surfaces (cosmos rows live in HostDetail, not Fleet) · `classifyNamed` one classifier for BOTH key sources (shadowed/unmatched badges) · keyed gallery: collision/unmatched/unknown-pending/unavailable/unrepresentable all honest · cross-platform stem rule · the deferred two-group ConfTab render test · icon matrix ×5 renderers; Codex READY WITH FIXES → wave → confirm ALL RESOLVED → residual copy fix; probe-verified live) — **owner eyeball DEFERRED-UNTIL-USE (standing ruling). D53 media-v2 = FEATURE-COMPLETE.**
...
- [ ] **S4-D76 — the call's few controls** (RULED 2026-09-25, [`D76`](./DECISIONS.md); spec = the D76 block in
