...
// (the sticky session pin when it is set — `/agent`, the gallery's Talk, the tools menu's agent rows,
// one seam, `pinSessionAgent` — else the OPEN THREAD's own D11 pin, else the resolved default); this
...
export function useActiveAgent(): string | null {
  ...
  return effectiveAgent(sticky, thread, agents ?? []);
}
