...
describe("planFrom", () => {
  it("prefers the executed result's data.plan", () => {
    ...
    const r = result("ok", {
      data: { plan: { steps: [{ text: "from result", status: "done" }] } },
    });
...
describe("pairResults", () => {
  it("pairs every tool result to its call by id across the thread", () => {
    const messages = [
      ...
      msg("t1", "tool", [
        { type: "tool_result", call_id: "c1", result: result("ok", { summary: "woke" }) },
      ]),
      msg("t2", "tool", [
        { type: "tool_result", call_id: "c2", result: result("error", { summary: "down" }) },
      ]),
    ];
  ...
  it("currentPlan tracks the MOST-RECENT task_plan call (the model rewrites the whole list each call)", () => {
    const messages = [
      ...
      msg("a2", "assistant", [
        call("p2", "task_plan", { steps: [{ text: "new", status: "pending" }] }),
      ]),
    ];
  ...
  it("currentPlan prefers the latest plan call's RESULT data over its args", () => {
    const messages = [
      msg("a1", "assistant", [
        call("p1", "task_plan", { steps: [{ text: "args", status: "pending" }] }),
      ]),
      msg("t1", "tool", [
        {
          type: "tool_result",
          call_id: "p1",
          result: result("ok", { data: { plan: { steps: [{ text: "result", status: "done" }] } } }),
        },
      ]),
    ];
    const { currentPlan } = pairResults(messages);
  ...
  it("currentPlan is null when the thread has no task_plan call", () => {
    const { currentPlan, resultByCall } = pairResults([
      msg("a1", "assistant", [call("c1", "wake_host")]),
    ]);
